...
import com.zhuoan.biz.event.SuperEventDeal;
 ...
 **/
@Component
public class DdzGameEventDsDeal extends SuperEventDeal {

    private final static Logger logger = LoggerFactory.getLogger(DdzGameEventDsDeal.class);
    ...
    public void createRoom(SocketIOClient client, Object data) {
        ...
        DdzGameRoom room = (DdzGameRoom) roomDeserializable(roomNo);
        if (room != null) {
            // 比赛场获取分数
    ...
    public void joinRoom(SocketIOClient client, Object data) {
        ...
        String roomNo = joinData.getString(CommonConstant.DATA_KEY_ROOM_NO);
        // 非重连通知其他玩家
        DdzGameRoom room = (DdzGameRoom) roomDeserializable(roomNo);
        if (room != null) {
            ...
            if (joinData.containsKey("isReconnect") && joinData.getInt("isReconnect") == 0) {
                JSONObject obj = obtainPlayerInfo(roomNo, account);
                // 通知玩家
    ...
    public void gameReady(SocketIOClient client, Object data) {
        JSONObject postData = JSONObject.fromObject(data);
        // 不满足准备条件直接忽略
        if (!checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_INIT, client) &&
            !checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_READY, client) &&
            !checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_SUMMARY, client)) {
            return;
        }
        // 房间号
    ...
    public void gameBeLandlord(SocketIOClient client, Object data) {
        JSONObject postData = JSONObject.fromObject(data);
        // 不满足准备条件直接忽略
        if (!checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_CHOICE_LANDLORD, client)) {
            return;
        }
        // 房间号
        final String roomNo = postData.getString(CommonConstant.DATA_KEY_ROOM_NO);
        ...
        if (isAllChoice(roomNo)) {
            ...
            //
            room = (DdzGameRoom) roomDeserializable(roomNo);
            if (isContinue) {
                beginTime = true;
                // 状态改变
    ...
    public void gameDouble(SocketIOClient client, Object data) {
        JSONObject postData = JSONObject.fromObject(data);
        // 不满足准备条件直接忽略
        if (!checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_DOUBLE, client)) {
            return;
        }
        // 房间号
        String roomNo = postData.getString(CommonConstant.DATA_KEY_ROOM_NO);
    ...
    public void gameEvent(SocketIOClient client, Object data) {
        JSONObject postData = JSONObject.fromObject(data);
        // 不满足准备条件直接忽略
        if (!checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_GAME_IN, client)) {
            return;
        }
        // 房间号
        final String roomNo = postData.getString(CommonConstant.DATA_KEY_ROOM_NO);
    ...
    public void getOutInfo(SocketIOClient client, Object data) {
        ...
        if (room.getRoomType() == CommonConstant.ROOM_TYPE_JB || room.getRoomType() == CommonConstant.ROOM_TYPE_YB) {
            ...
            if (room.getUserPacketMap().get(account).getStatus() == DdzConstant.DDZ_USER_STATUS_INIT) {
                ...
                room.getGameStatus() == DdzConstant.DDZ_GAME_STATUS_SUMMARY) {
                // 初始及准备阶段可以退出
                canExit = true;
            }
        }
    ...
    public void gameContinue(SocketIOClient client, Object data) {
        JSONObject postData = JSONObject.fromObject(data);
        // 不满足准备条件直接忽略
        if (!checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_SUMMARY, client) &&
            !checkGameEvent(postData, DdzConstant.DDZ_GAME_STATUS_READY, client)) {
            //return;
        }
        // 房间号
    ...
    public void reconnectGame(SocketIOClient client, Object data) {
        ...
        DdzGameRoom room = (DdzGameRoom) roomDeserializable(roomNo);
        // 房间不存在或不在当前房间内
        if (room == null || Dto.stringIsNULL(account) || !room.getPlayerMap().containsKey(account) || room.getPlayerMap().get(account) == null) {
            result.put("type", 0);
            CommonConstant.sendMsgEventToSingle(client, String.valueOf(result), "reconnectGamePush_DDZ");
            return;
        }
    ...
    private String getFirstAccountInRoom(String roomNo) {
        try {
            // 房间实体对象
            final DdzGameRoom room = (DdzGameRoom) roomDeserializable(roomNo);
            if (room != null) {
                // 当前房间内的所有的所有玩家
