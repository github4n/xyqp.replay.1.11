...
public class SSSComputeCards {
	 ...
	 * @param card1
	 * @param card2
	 * @return [["A:-1","B:+1","SpeA:+0","SpeB:+0"],["A:+1","B:-1","SpeA:+1","SpeB:-1"],["A:+1","B:-1","SpeA:+4","SpeB:-4"],["sumA:+1","sumB:-1"]]
	 */
/*	public static JSONArray compare(JSONArray card1,JSONArray card2) {
		
		JSONArray result=new JSONArray();
		
		for (int k=0;k<card1.size();k++) {
			JSONArray player1=(JSONArray) card1.get(k);
			JSONArray player2=(JSONArray) card2.get(k);

			JSONObject aJsonObject=new JSONObject();
			JSONObject bJsonObject=new JSONObject();
			
			int flushByflowerPlayer1=isFlushByFlower(player1);
			...
			int flushByflowerPlayer2=isFlushByFlower(player2);
			...
			if(flushByflowerPlayer1>flushByflowerPlayer2){
				...
				if(flushByflowerPlayer1>0){
					...
					if(bombPlayer1>bombPlayer2){
						...
						if(bombPlayer1>0){
							if(compareBomb(player1,player2)){
								temp.add("A:+1");
								temp.add("B:-1");
								if(k==1){
									temp.add("SpeA:+6");
							...
							if(gourdPlayer1>gourdPlayer2){
								...
								if(gourdPlayer1>0){
									if(compareGourd(player1,player2)){
										temp.add("A:+1");
										temp.add("B:-1");
										if(k==1){
											temp.add("SpeA:+1");
											...
											if(flushPlayer1>flushPlayer2){
												...
												if(flushPlayer1>0){
													...
													if(threePlayer1>threePlayer2){
														...
														if(threePlayer1>0){
															if(compareThree(player1,player2)){
																temp.add("A:+1");
																temp.add("B:-1");
																if(k==0){
																	temp.add("SpeA:+2");
		...
		while (iterator.hasNext()) {
			JSONArray temp=iterator.next();
			String a=temp.getString(0);
			String b=temp.getString(1);
			sumA+=Integer.parseInt(a.split(":")[1]);
			sumB+=Integer.parseInt(b.split(":")[1]);
		}
